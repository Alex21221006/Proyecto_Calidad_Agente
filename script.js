...
/* Botones de lupa: consultar automáticamente a la API local Flask */
document.addEventListener('click', async (ev) => {
  ...
  try {
    ...
    if (!res.ok) {
      alert(`Error del backend (${res.status}). ${data?.error || "Inténtalo más tarde."}`);
      return;
    }
    ...
    console.error(err);
    alert("No fue posible contactar al backend. ¿Está Flask ejecutándose en http://127.0.0.1:5000?");
  }
});
...
q('#btnSave').addEventListener('click', () => {
  ...
  const oper   = q('#agOperator').value.trim(); // operador = usuario logueado
  ...
  generatePDF(rec); // abre nueva pestaña
  q('#btnClear').click();
  alert('Movimiento guardado y boleta generada.');
});
